...
_local_netref_attrs = frozenset([
    '____conn__', '____oid__', '____local_cache__', '____async_wrapper__', '__class__', '__cmp__', '__del__', '__delattr__',
...
def syncreq(proxy, handler, *args):
    ...
    try:
        ...
        result = conn.sync_request(handler, oid, *args)
    finally:
...
class BaseNetref(object):
    ...
    :param cache_textual_attribs: enable caching of attributes that are usually textually defined (and therefore tend to remain static), such as instance methods, modules, etc.
    """
    # this is okay with py3k -- see below
    __metaclass__ = NetrefMetaclass
    __slots__ = ["____conn__", "____oid__", "____local_cache__", "____async_wrapper__", "__weakref__"]
    def __init__(self, conn, oid, cache_textual_attribs=True):
        self.____conn__ = conn
        self.____oid__ = oid
        self.____local_cache__ = {} if cache_textual_attribs else None
        self.____async_wrapper__ = None
    ...
    def __getattribute__(self, name):
        if name in _local_netref_attrs:
            ...
            try:
                # look up method wrappers from the class 
                result = object.__getattribute__(self, name)
                # optionally wrap into async wrapper 
                if self.____async_wrapper__ is not None:
                    return self.____async_wrapper__(result)
                else:
                    return result
            except AttributeError:
                # look up instance variables and the like via __getattr__
                result = self.__getattr__(name)
                # wrap into async wrapper if this is actually a method (perhaps added dynamically)
                if self.____async_wrapper__ is not None and (isinstance(result,types.MethodType) or isinstance(result,types.BuiltinMethodType)):
                    return self.____async_wrapper__(result)
                else:
                    return result
                
    def __getattr__(self, name):
       # get attributes from the remote instance
        if self.____local_cache__ is None:
            # cache disabled -- use default behavior
            return syncreq(self, consts.HANDLE_GETATTR, name)
        else:
            # caching of methods enabled
            if name in self.____local_cache__:
                # we have seen the name before 
                if self.____local_cache__[name] is False:
                    # but it was not cachable: request
                    return syncreq(self, consts.HANDLE_GETATTR, name)
                else:
                    return self.____local_cache__[name]
            else:
                # this is the first time that we see this name
                result = syncreq(self, consts.HANDLE_GETATTR, name)
                if inspect.ismethod(result) or inspect.isfunction(result) or inspect.isclass(result) or inspect.ismodule(result):
                    self.____local_cache__[name] = result
                else:
                    self.____local_cache__[name] = False
                return result
    ...
    def __iter__(self):
        def buffered_iterator(obj,chunk=64,max_chunk=1024,factor=2):
            """An iterable generator that pulls elements on a chunk-by-chunk basis over the network."""
            if factor < 1:
                raise ValueError("factor must be >= 1, got %r" % (factor,))
            it = syncreq(self, consts.HANDLE_ITER)
            count = chunk
            while True:
                items = syncreq(it, consts.HANDLE_BUFFITER, count)
                count = min(count * factor, max_chunk)
                for elem in items:
                    yield elem
                if len(items) < count:
                    break
        return buffered_iterator(self)    
...
def method_definitions(methods):
    """Turns a dictionary of method names and docstrings into a dictionary of method actual (remote) definitions.""" 
    ns = {"__slots__" : ()}
...
def class_factory(clsname, modname, methods):
    ...
    if modname in sys.modules and hasattr(sys.modules[modname], clsname):
        ...
        ns["__class__"] = None
    cls = type(clsname, (BaseNetref,), ns)
    return cls
