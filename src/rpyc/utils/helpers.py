...
from rpyc.core.consts import HANDLE_BUFFITER, HANDLE_CALL, HANDLE_CALLATTR 
...
class _AsyncMethod(object):
    """Creates an async proxy wrapper over an existing locally represented method. Async proxies
    are cached. Invoking an async proxy will return an AsyncResult instead of
    blocking"""
    ...
    def __init__(self, proxy):
        self.proxy = proxy
    def __call__(self, *args, **kwargs):
        return asyncreq(self.proxy.__self__, HANDLE_CALLATTR, self.proxy.__name__, args, tuple(kwargs.items()))
    def __repr__(self):
        return "async(%r)" % (self.proxy,)

class _DummyAsync(object):
    """Creates a dummy async proxy wrapper over an existing proxy. Async proxies
    are cached. Invoking an async proxy will return an AsyncResult instead of
    blocking"""
    __slots__ = ("proxy", "__weakref__")
    def __init__(self, proxy):
    ...
    def __repr__(self):
        return "dummy-async(%r)" % (self.proxy,)
...
def async(proxy):
    ...
    if hasattr(proxy, "____conn__") and hasattr(proxy, "____oid__"):
        caller = _Async(proxy)
        _async_proxies_cache[id(caller)] = _async_proxies_cache[pid] = caller
    elif hasattr(proxy, "__self__") and hasattr(proxy, "__name__") and hasattr(proxy.__self__, "____conn__") and hasattr(proxy.__self__, "____oid__"):
        caller = _AsyncMethod(proxy)
    else:
        caller = _DummyAsync(proxy)
    return caller

async.__doc__ = _Async.__doc__

def async_value(obj):
    """Get the value associated with an async result. Works even if the object under consideration is not an async result.""" 
    try:
        return obj.async_value
    except:
        return obj

def enable_local_cache(proxy,enabled=True):
    """Enables automatic local caching of function, method, class and module attributes on the given proxy."""
    try:
        proxy.____local_cache__ = {} if enabled else None
    except:
        pass
    return proxy

def enable_async_methods(proxy,enabled=True):
    """Enables automatic asynchronous return values for remote methods on the given proxy."""
    try:
        proxy.____async_wrapper__ = async if enabled else None
    except:
        pass
    return proxy
